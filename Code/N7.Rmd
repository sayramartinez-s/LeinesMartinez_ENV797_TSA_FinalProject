...
```{r}
p7 <- autoplot(ts_oil_total) +
  #autolayer(ts_oil_b043, series="043",color="red")+
  labs(color = "Series", title = "Oil Production", x = "Time", y = "ts_objects") +
  theme(
  ...
  guides(color = guide_legend(override.aes = list(color = "red")))


print(p7)

output_file1 <- file.path(output_dir, "ts_oil_production_total.png")
ggsave(filename = output_file1, plot = p7, width = 10, height = 6, dpi = 300, bg = "transparent")
```

```{r}
p6 <- p6 +
  labs(color = "Series", title = "Oil price", x = "Time", y = "ts_price") +
  theme(
    text = element_text(size = 14),
    plot.title = element_text(size = 18, face = "bold"),
    axis.title = element_text(size = 16),
  ...
  guides(color = guide_legend(override.aes = list(color = "darkgreen")))


print(p6)

output_file2 <- file.path(output_dir, "ts_oil_prices.png")
ggsave(filename = output_file2, plot = p6, width = 10, height = 6, dpi = 300, bg = "transparent")
```


```{r}
# ACF and PACF for Production
p8 <- as.ggplot(function() {
  Acf(ts_oil_total, lag.max = 40, main = "ACF of Total Production")
})

output_file3 <- file.path(output_dir, "ACF_oil_production.png")
ggsave(filename = output_file3, plot = p8, width = 10, height = 6, dpi = 300, bg = "transparent")

p9<- as.ggplot(function() {
  Pacf(ts_oil_total, lag.max = 40, main = "PACF of Total Production")
})

output_file4 <- file.path(output_dir, "PACF_oil_production.png")
ggsave(filename = output_file4, plot = p9, width = 10, height = 6, dpi = 300, bg = "transparent")
