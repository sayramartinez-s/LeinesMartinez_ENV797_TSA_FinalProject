...
```{r}
# Model A: Baseline SARIMA on total production
model_1_train <- auto.arima(ts_train_C, seasonal = TRUE)
...
```{r}
# Model 3: ARIMAX on deseasonalized production with regressors
model_3_train <- auto.arima(ts_train_C, xreg = xreg_train, seasonal = TRUE)
...
```{r}
# Model 4
model_4_train <- meanf(ts_train_C, h = h)
...
```{r}
# Model 6: 
model_6_train <- tbats(ts_train_C)
...
```{r}
# Model 7: 
model_7_train <- nnetar(ts_train_C)
...
```{r}
# Model 8: 
model_8_train <- StructTS(ts_train_C, type = "BSM")
...
#Model 1
SARIMA_scores <- accuracy(forecast_1$mean, ts_test_C)
...
#Model 3
ARIMAX_scores <- accuracy(forecast_3$mean, ts_test_C)

#Model 4
Mean_scores <- accuracy(model_4_train$mean, ts_test_C)

#Model 5
ETS_scores <- accuracy(forecast_5$mean, ts_test_C)

#Model 6
TBATS_scores <- accuracy(forecast_6$mean, ts_test_C)

#Model 7
NN_scores <- accuracy(forecast_7$mean, ts_test_C)

#Model 8
StructTS_scores <- accuracy(forecast_8$mean, ts_test_C)
